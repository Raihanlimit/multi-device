...
export function getMediaKeys(buffer, mediaType: MediaType) {
   ...
   const expandedMediaKey = hkdf(buffer, 112, {
...
export async function getAudioDuration(buffer: Buffer | string | Readable) {
   ...
   if (Buffer.isBuffer(buffer)) {
      metadata = await musicMetadata.parseBuffer(buffer, null, {
         duration: true
      })
   } else if (typeof buffer === 'string') {
      const rStream = createReadStream(buffer)
      metadata = await musicMetadata.parseStream(rStream, null, {
         duration: true
      })
      rStream.close()
   } else {
      metadata = await musicMetadata.parseStream(buffer, null, {
         duration: true
      })
   }
...
export const getStream = async (item: WAMediaUpload) => {
   ...
   if (item.url.toString().startsWith('http://') || item.url.toString().startsWith('https://')) {
      ...
      return {
         stream: await getGotStream(item.url),
         type: 'remote'
      }
   }

   return {
      stream: createReadStream(item.url),
      type: 'file'
   }
}
...
export const getGotStream = async (url: string | URL, options: Options & {
   isStream ? : true
} = {}) => {
   const {
      default: {
   ...
   const fetched = gotStream(url, {
      ...options,
      isStream: true
   })
   await new Promise((resolve, reject) => {
      fetched.once('error', reject)
      fetched.once('response', ({
         statusCode
      }: Response) => {
         if (statusCode >= 400) {
            reject(
...
export const encryptedStream = async (
   ...
   const encWriteStream = new Readable({
      read: () => {}
   })
...
export const getWAUploadToServer = ({
   ...
   fetchAgent,
   logger
}: CommonSocketConfig < any > , refreshMediaConn: (force: boolean) => Promise < MediaConnInfo > ): WAMediaUploadFunction => {
   return async (stream, {
      mediaType,
      fileEncSha256B64,
      timeoutMs
